...
class ULTerm
    ...
    # Transfer integers to corresponding alphabets
    def getAlphabet(n)
...
class ULVar < ULTerm
    ...
    # Method prettify for ULVar
    def prettify
        getAlphabet(self.index)
    end
...
class ULAbs < ULTerm
    ...
    # Construct string with format "lambda _ . "
    def lambdaConstruct(n) 
        "lambda " + (96 + n).chr + " . "
    end

    # Method prettify for ULAbs
    def prettify
        lambdaConstruct(lambdaNum(self.t) + 1) + self.t.prettify
    end
end
...
class ULApp < ULTerm
    ...
    # Method prettify for ULApp
    def prettify
        "(" + self.t1.prettify + ") (" + self.t2.prettify + ")"
    end    
end

# Count the number of ULAbs in an expression
def lambdaNum(expr)
    case expr
    when ULVar
        0
    when ULAbs
        lambdaNum(simplifyAbs(expr)) + 1
    when ULApp
        0
    end
end

# Simplify one level of ULAbs
def simplifyAbs(expr)
    expr.t
end
