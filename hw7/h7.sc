...
// Method prettify
def prettify(t: ULTerm): String = t match {
  case ULVar(a) => getAlphabet(a)
  case ULAbs(a) => lambdaConstruct(lambdaNum(a) + 1) + prettify(a)
  case ULApp(a,b) => "(" + prettify(a) + ") (" + prettify(b) + ")"
}

// Helper function that can count the number of ULAbs in a ULTerm expression
def lambdaNum(t: ULTerm): Int = {
  var counter = 0
  def simplify(t: ULTerm): Int = t match {
    case ULVar(_) => counter
    case ULAbs(a) =>
      counter += 1
      simplify(a)
    case ULApp(a,b) =>
...
def lambdaConstruct(n: Int): String = {
  "lambda " + (96 + n).toChar.toString() + " . "
}

def getAlphabet(input: Int): String = {
